...
import { Bell, Gauge, Moon, Ruler, Settings2, Sun, Volume2 } from "lucide-react";
...
type SettingsPanelProps = {
  ...
  onAnimationIntensityChange: (intensity: ReminderSettings["animationIntensity"]) => void;
    ...
    <div className="space-y-3">
      ...
      <AnimatePresence>
        {isOpen ? (
          ...
          >
            <motion.aside
              className="absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-slate-950/88 p-4 shadow-[0_0_80px_rgba(8,47,73,0.45)] backdrop-blur-2xl sm:p-6"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              ...
              <div>
                ...
                <div className="mt-4 flex items-center gap-3">
                  <input
                    ...
                    onChange={(event) =>
                      onReminderFrequencyChange(Number(event.target.value))
                    }
                    className="w-full accent-cyan-300"
                  />
